    ...
    def load_model(self):
        ...
        try:
            ...
            if self.vectorizer is not None:
                # If unpickling failed to restore idf_ attribute
                if not hasattr(self.vectorizer, "idf_"):
                    logger.warning("Vectorizer IDF vector is missing. Initializing dummy IDF for stability.")
                    # Initialize a dummy IDF of 1.0 for all features if missing
                    # This prevents the 'NotFittedError' while maintaining basic keyword detection
